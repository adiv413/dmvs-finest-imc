        ...
        for product in self.products:
            ...
            if product == 'AMETHYSTS':
                ...
                if len(order_depth.sell_orders) > 0 and len(order_depth.buy_orders) > 0:
                    ...
                    # first n iterations
                    if len(PREV_PRICES) < LR_LOOKBACK + 1:
                    ...
                    prev_percentages = [(PREV_PRICES[i + 1] - PREV_PRICES[i])/PREV_PRICES[i] + 1 for i in range(len(PREV_PRICES) - 1)]


                    m, b = linear_regression(PREV_TIMESTAMPS[1:], prev_percentages)

                    next_time = state.timestamp + (PREV_TIMESTAMPS[-1] - PREV_TIMESTAMPS[-2])

                    change_in_price = m * next_time + b

                    acceptable_price = change_in_price * curr_price
                    ...
                    ## ALGO ORDERS
                    if best_ask < (acceptable_price - 3) and best_ask != buy_quote: #second part of and is so that orders don't overlap, which lets me individually keep track of positions
                    ...
                    if best_bid > (acceptable_price + 3) and best_bid != sell_quote: #second part of and is so that orders don't overlap, which lets me individually keep track of positions
